...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ // Gibbs sampler for Ising
    for(int i = 0; i < _npart; i++){
      double h = _particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin();
      double P = 1./(1+exp(-2*_beta*_J*h));
      if(_rnd.Rannyu() < P){
    ...
    if(_sim_type == 1){       // LJ system
      ...
      for(int j=0; j<_ndim; j++){
        shift(j) = _rnd.Rannyu(-1,1) * _delta; // Uniform distribution in [-_delta;_delta)
      }
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    } else if( property == "THERMALIZATION"){
      input >> _therma;;
      coutf << "THERMALIZATION= " << _therma << endl;
  ...
  if (input.is_open()){
    while ( input >> property ){
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutt << "#     TEMP:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        index_property++;
        _ptail = (32*M_PI*pow(_rho, 2))/(9*pow(_r_cut, 9)) - (16*M_PI*pow(_rho, 2))/(3*pow(_r_cut, 3));
        ...
        coutpr << "#     TEMP:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << "#     TEMP:   ACTUAL_CV:    CV_AVE:      ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        coutpr << "#     TEMP:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        for (int i_bin = 0; i_bin < _n_bins; i_bin ++) {
          if (dr < (i_bin + 1) * _bin_size && dr >= i_bin * _bin_size) {
            _measurement(_index_gofr + i_bin)+=2;
            break;
          }
        } 
        if(dr < _r_cut){
    ...
    for(int i_bin = 0; i_bin < _n_bins; i_bin ++){
      double r_low = i_bin * _bin_size;
      double r_up = (i_bin + 1) * _bin_size;

      double shell_volume = (4.0 / 3.0) * M_PI * (pow(r_up, 3) - pow(r_low, 3));
      double normalization = _rho * _npart * shell_volume;

      _measurement(_index_gofr + i_bin) /= normalization;
    }
  }
  // POFV //////////////////////////////////////////////////////////////////////
  if (_measure_pofv){
    ...
    for(int i = 0; i < _npart; i ++){
       ...
       double mod_v = sqrt(v_x*v_x + v_y*v_y + v_z*v_z); // Compute the velocity module for particle i
        for (int i_bin = 0; i_bin < _n_bins_v; i_bin ++) {
          if (mod_v < (i_bin + 1) * _bin_size_v && mod_v >= i_bin * _bin_size_v) {
            _measurement(_index_pofv + i_bin)++;
            break;
        }
      }
    }
    ...
    for(int i_bin = 0; i_bin < _n_bins_v; i_bin ++){
      ...
      // Normalization on the total number of particles, bin amplitude, and density of states
  ...
  // TOTAL ENERGY (kinetic+potential), SPECIFIC HEAT ///////////////////////////
  if (_measure_tenergy or _measure_cv){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      for (int i=0; i<_npart; i++){
        ...
        double Eij = - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
        tenergy_temp += Eij;
        cv += Eij*Eij;
      }
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure) _measurement[_index_pressure] = _rho * _temp + (48.0*virial/3.0)/_volume + _ptail;
  // MAGNETIZATION, SUSCEPTIBILITY /////////////////////////////////////////////
  if(_measure_magnet or _measure_chi) {
    for(int i = 0; i < _npart; i ++){
      double si=_particle(i).getspin();
      magnetization += si;
    }
    double M2 = magnetization * magnetization;
    _measurement(_index_magnet) = magnetization / _npart;
    _measurement(_index_chi) = _beta*M2/_npart;
  }
...
void System :: averages(int blk){
  ...
  if (_measure_cv){
  double H2 = _average(_index_cv);
  double H  = _average(_index_tenergy);
  _average(_index_cv) = pow(_beta, 2) * (H2 - H * H);
  }

  if (_measure_chi){
  double M2 = _average(_index_chi);
  double M  = _average(_index_magnet)*_npart;
  _average(_index_chi) = _beta/double(_npart) * (M2 - M * M);
  }
  ...
  if (_measure_magnet){
    ...
    if (blk == _nblocks) coutf << setw(12) << _temp
                               << setw(12) << average
                               << setw(12) << sum_average/double(blk)
                               << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
