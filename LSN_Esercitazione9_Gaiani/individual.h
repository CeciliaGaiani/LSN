...
// The individual class allows for the construction of the succession of 34 cities to be visited (chromosomes of the genetic algorithm)
...
// Operator that initializes individuals with cities randomly distributed over a sphere of radius "radius"
...
// Operator that initializes individual with cities randomly distributed in a square of side "side"
...
// Method that allows to initialize the individual using initialize_individual_circumference or initialize_individual_square
  void initialize_individual(Random& rnd, string shape, double radius, double side){
    if(shape == "circle"){
      initialize_individual_circumference(rnd, radius);
    } else if(shape == "square"){
      initialize_individual_square(rnd, side);
    } else {
      cout << "Unrecognised shape." << endl;
    }
...
void save_individual(string filename){
...
// Method that corrects the individual if it presents repeated cities (I already assume that the check_index_cities function has found equal indexes)
  ...
  for (int i = 0; i < ncities; i++) {
    ...
    count[ind]++;                          // The i-th component of the "count" vector is incremented by one each time of index i is found          
    if (count[ind] > 1) {
  ...
  size_t min_size = min(duplicate_positions.size(), missing_indices.size()); // Just in case duplicate_positions and missing_indices have different dimension
