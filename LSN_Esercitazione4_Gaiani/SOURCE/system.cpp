...
void System :: step(){ // Perform a simulation step
  if(_sim_type == 0) this->ReverseVerlet();  // Perform a MD step
...
void System::BeginReverseOnce() {
  if (_reverse) return;

  for (int i = 0; i < _npart; ++i) {
    // Read configurations
    const double cx = _particle(i).getposition(0, true);   // x_t (conf.xyz)
    const double cy = _particle(i).getposition(1, true);
    const double cz = _particle(i).getposition(2, true);

    const double ox = _particle(i).getposition(0, false);  // x_{t-Δt} (conf-1.xyz)
    const double oy = _particle(i).getposition(1, false);
    const double oz = _particle(i).getposition(2, false);

    // Set old = x_t
    _particle(i).setposition(0, cx);
    _particle(i).setposition(1, cy);
    _particle(i).setposition(2, cz);
    _particle(i).acceptmove();

    // Set current = x_{t-Δt}
    _particle(i).setposition(0, ox);
    _particle(i).setposition(1, oy);
    _particle(i).setposition(2, oz);

    _particle(i).setvelocity(0, this->pbc(ox - cx, 0) / (2.0 * _delta));
    _particle(i).setvelocity(1, this->pbc(oy - cy, 1) / (2.0 * _delta));
    _particle(i).setvelocity(2, this->pbc(oz - cz, 2) / (2.0 * _delta));
  }
...
void System::ReverseVerlet() {
  if (!_reverse) BeginReverseOnce();  // Reverse configurations
  Verlet();                           // to integrate backwards
}
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    cinf.close();
  } else { // Implementation of a velocity distribution represented by a Dirac 𝛿 function centered at v_T
    ...
    for (int i=0; i<_npart; i++){
      ...
      vy(i) = 0.0;
      vz(i) = 0.0;
      double which_comp = rnd.Rannyu(0, 1);
      double sign_v = rnd.Rannyu(0, 1);
      double v_T = sqrt(2*_temp);

      if(which_comp < 0.33){
        if(sign_v < 0.5) vx(i) = v_T;
        else vx(i) = - v_T;
      } else if (which_comp >= 0.33 && which_comp < 0.66){
        if(sign_v < 0.5) vy(i) = v_T;
        else vy(i) = - v_T;
      } else {
        if(sign_v < 0.5) vz(i) = v_T;
        else vz(i) = - v_T;
      }
