...
bool Random::Initialize(const string& seed_file, const string& primes_file, int rank) {
    int seed[4];
    int p1, p2;

    std::ifstream Primes(primes_file);
    if (!Primes.is_open()) {
        std::cerr << "PROBLEM: Unable to open " << primes_file << std::endl;
        return false;
    }
    Primes >> p1 >> p2;
    Primes.close();
    ...
    if (!input.is_open()) {
        std::cerr << "PROBLEM: Unable to open " << seed_file << std::endl;
        return false;
    }
